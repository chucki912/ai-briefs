...
**검색 최신성 규칙**: 벤치마크 점수·리더보드 순위·제품 버전·점유율 수치는 인용 전 최신값을 재검색으로 확인할 것. 검색에 잡힌 수치가 발표 후 2개월 이상 경과했으면 그 사이 갱신이 없는지 1회 추가 확인 — 구 스냅샷을 현재값으로 서술 금지. 갱신 확인 불가 시 수치에 기준 시점을 명시할 것('2026-05 발표 기준').
...
9. **당사자 프레임 검증**: 이해당사자의 자기 서사(벤더의 영업 화법·경쟁사 비교, 소송 당사자의 주장, 기업의 자체 성과 발표)를 인용할 때는 반드시 (a) 반대 당사자의 반박 또는 (b) 독립 제3자의 유보·검증 중 최소 1개를 같은 섹션에 결합할 것. 결합할 수 없으면 해당 주장을 '~라고 주장함' 수준의 귀속 표현으로 한정하고 리포트의 논지로 승격하지 말 것. 거래·협상·제휴 보도에서 제안 주체와 검토/수락 주체를 뒤바꾸지 말 것 — 누가 먼저 원했는가는 거래 해석의 핵심 팩트임.
10. **킬트리거 타당성**: 폐기 조건(killTrigger)은 '논지가 참일 때 정상적으로 통과 가능한 조건'이어야 함. 사건의 현재 단계와 조건의 시간 축을 맞출 것 — 초기 협상 단계 사안에 차기 분기 실적 공시를 폐기 조건으로 삼으면 참인 논지도 조기 기각됨. 자가 검증: 이 논지가 맞다면 이 조건을 통과하는 현실적 경로가 존재하는가?
