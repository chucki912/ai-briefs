...
- **포트 선점 주의:** 로컬 구동 시 3000번 포트가 이미 사용 중인 경우, 서버는 자동으로 **3001번 포트**로 구동될 수 있습니다. `npm run dev` 로그에서 포트를 확인한 뒤 `http://localhost:3001/api/generate` 등으로 적절히 변경하여 호출하십시오.
...
7. **[4중 중복 검증]** headline(사건), oneLineSummary(단일 논지), keyFacts(건조한 팩트), insight(시사점) 간의 텍스트 및 의미 중복이 배제되었는가? (헤드라인을 그대로 팩트에 복사하거나 요약과 인사이트가 되풀이되는 문구는 반려 대상입니다.)
...
4. **[최상급 수사 검증]** 리포트 전반에 "세계 최고의", "가장 파괴적인", "초격차", "게임체인저", "거대한 물줄기" 등 자극적이고 과장된 수사가 배제되고 중립적 서술로 채워졌는가?

### 🔍 평문 다이제스트 및 호환성 검증
생성된 마크다운 다이제스트(`generateMarkdown` 아웃풋)를 열고 다음 항목을 확인하세요:
1. 개별 이슈 하단에 `논지: {oneLineSummary}`가 정상 노출되는가?
2. `soWhat`이 있는 이슈의 경우 `So What:` 하위에 4분 구조 평문 블록이 정상 노출되는가?
3. **[하위 호환성 검증]** `soWhat`이 존재하지 않는 과거 Redis 캐시 데이터를 렌더링할 때 에러로 인해 시스템이 중단되지 않고 `soWhat` 블록이 정상적으로 스킵(생략)되는가?

## Step 4. UI 렌더링 확인
위의 JSON이 완벽하다면 비로소 웹 브라우저 (http://localhost:3000 또는 3001)를 띄우고 UI 렌더링 시 디자인 오류나 텍스트 짤림이 없는지 육안으로 최종 확정합니다.
- `soWhat` 블록의 4분 면이 모던하고 정갈하게 정렬되는지 확인하십시오.
- `oneLineSummary` (논지) 영역의 폰트 및 테두리 스타일이 명제 형태로 시각화 강조되었는지 확인하십시오.
