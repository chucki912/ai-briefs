...
9. 문체: 모든 필드 텍스트는 개조식 종결(~함/~임/~전망)로 작성. 서술형 종결(~습니다/~한다/~있다) 금지. 초안이 서술형이면 개조식으로 변환하되 내용은 변경 금지. 이미 명사(구)로 끝나는 문장에는 '~임'을 덧붙이지 말 것 (예: '리소스 낭비 발생' ○ / '리소스 낭비 발생임' ×). '~임/~함'은 서술형 문장을 개조식으로 종결할 때만 사용.
...
12. anchor.value와 anchor.source에 매체명·사이트명을 나열하지 말 것. anchor.source에는 원출처 성격의 단일 표기만(예: 'MS Q3 FY26 실적발표'), 출처 식별은 sourceIds 결박으로만 표현됨.
13. soWhat.action.costIfWrong/costIfMissed와 risks[].downsideCost는 서로 다른 층위임 — 전자는 '독자의 대응 베팅'이 틀렸을 때의 비용, 후자는 '분석 대상 기업'이 지는 비용. 같은 문장의 복제 금지. 초안에 한 층위만 있으면 다른 층위는 그 관점으로 재기술할 것.`;
