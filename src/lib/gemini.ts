...

{
  ...
  "singleTopicStatement": "단일 주제 (50자 이내): [기업/이벤트]이 [메커니즘]을 통해 [글로벌 산업 구조의 변화]",
  ...
  "insight": "[산업 구조 변화 판단 1문장] [이 이슈가 만드는 압력 1문장] [핵심 시사점 1문장] 형식의 심층 인사이트 (최대 3문장)",
...
다음 템플릿을 채워라: "[선택한 단 1개의 사건]이 [구체적 메커니즘]을 통해 [글로벌 산업 구조의 구체적 변화]"

예시 ✅: "MS의 오피스 2024 AI 기본 탑재 할인이(단일 사건) 묶음 판매 전략(메커니즘)을 통해 글로벌 소프트웨어 수익 구조를 재편함(변화)"
...
- **팩트 원칙**: 기업명, 제품명, 모델명은 수집된 원문에 등장한 명칭만 사용할 것. 원문에 없는 고유명사 생성 금지. 불확실한 수치는 생략하고 확인된 팩트만 서술함.
- 출력 시 각 Fact 문장 뒤에 "| 메커니즘: [확인 내용]" 형식으로 동일성 명시

### STEP 4. 시리즈 컨텍스트 빌드업 (시리즈 컨텍스트가 있을 때 필수)
이전 브리프들의 요약(마스터 논지, 이전 처방 등)을 읽고 **유기적으로 연결되는 서사(Storyline)**를 구상하라.
- ❌ 금지: 이전 브리프와 완전히 단절된 독립적 분석
- ✅ 필수: 이전 브리프가 다룬 위협/기회와 **어떻게 맞물려 거대한 파도를 형성하는지** 맥락적으로 융합할 것 (예: "앞선 앤스로픽의 보안 이슈에 더해, 이번 MS의 행보는...")
- prescriptionLevel을 이전 브리프와 겹치지 않게 분화 (전략 → 전술 → 실행)

### STEP 5. insight 작성 (★구조 강제★)
반드시 아래 3문장 템플릿 구조를 유지하며, 내용은 수집된 팩트에서만 추출할 것:

1. [이 이슈가 만드는 글로벌 산업 구조 변화 1문장].
2. [그 변화가 만드는 압력 또는 산업적 역설 1문장].
3. [구체적 직무/산업군]은 [단기 1개월/중기 분기] 내에 [명사]를 [동사형 액션]하라.

**처방 작성 예시 (Few-shot):**
- ✅ "엔터프라이즈 구매 담당자는 이번 분기 내 벤더 락인 조항을 재검토하라."
- ✅ "B2B SaaS 마케팅 팀은 향후 1개월 내 타겟 메시징에서 '효율성' 키워드를 '보안성'으로 교체하라."
- ❌ "기업은 AI 도입 전략을 재수립해야 함." (뜬구름 처방 금지)

추가 조건:
- insight는 지시된 3문장 템플릿을 엄격히 따라야 하며, 축약형이 아닌 완성형 문장으로 작성할 것.
- **금지 표현**: "~의 일환임", "~을 상징함", "~로 전환이 시급함" 같은 선언형 마무리 절대 금지.
- **단어 제한**: "주권", "패권", "지정학" 단어는 단락 내 최대 1회만 허용.
- 적용 분석 프레임워크의 핵심 개념어를 최소 1회 명시 사용.
- **자연스러운 맥락 전환**: 이전 브리프의 논지와 연결점이 있다면, 억지스러운 괄호 표기 없이 자연스러운 서술어(예: "MS의 확장과 맞물려")를 활용하여 맥락을 이어갈 것.
- ❌ **순서 암시 단어 절대 금지**: "앞선 브리프에서", "이번에 논의된" 등 브리프의 읽는 순서를 단정 짓는 단어는 절대 쓰지 말고 수평적/동시다발적 인과로만 서술할 것.
- ❌ 절대 금지: 문장 끝에 기계적으로 "(※ trade-off: ...)"를 붙이는 행위
- 분량: 200자 이내 준수.
- 감정적 표현 배제, 개조식 축약 문체 유지 (insight 제외).
- **insight는 지시된 문장 구조를 따라야 하므로 축약형이 아닌 완성형 문장으로 작성할 것.**
...
[ ] 1. 클러스터 내 수많은 사건 중 오직 '단 1개의 강력한 사건'만 골라내어 singleTopicStatement를 작성했는가?
[ ] 2. excludedFacts에 선택되지 않은 나머지 사건들이 잘 버려져 있는가?
[ ] 3. keyFacts가 '단 1개의 사건'만을 3문장으로 쪼개서 상세히 설명하고 있는가?
[ ] 4. insight가 [산업 변화] + [압력] + [처방]의 3문장 구조인가?
[ ] 5. insight에 "주권/패권/지정학" 단어가 각각 1회 이하로 쓰였는가?
[ ] 6. insight의 처방에 구체적 주어(산업·직무)와 시간 범위, 동사형 마무리가 있는가?
[ ] 7. 원문에 없는 고유명사나 불확실한 수치를 임의로 생성하지 않았는가?
[ ] 8. insight의 선언형 마무리나 뜬구름 처방이 없는가?
[ ] 9. category가 허용 목록 내 1개인가?
[ ] 10. relevantSourceIndices가 keyFacts 출처만 포함하는가?

**★ 위 3, 4, 6 항목 중 하나라도 실패하면 해당 섹션을 즉시 재생성할 것 ★**
...
- **[So What]** <기존 산업 참여자들이 즉각 주목해야 할 구조적 임계점과 전략적 로드맵 제시>
