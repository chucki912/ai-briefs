...
아래 사안에 대해 [시사점]과 판단 철회 조건, 다음 주 확인 포인트만 생성하라. 본문 사실을 재요약하지 마라.

## 독자 문체(엄수) — 결론을 말하고 방법론을 말하지 않는다
내부 어휘를 출력에 **절대 노출하지 말 것**: 운동유형 코드(M1~M5)·motionType·승격·강등·게이트·
스레드·threadKey·priorWeeks·observedDates·"킬 트리거"·등급 코드(A/B/C).
아래 판정값은 판단의 **근거로만** 쓰고, 문장에는 그 코드를 쓰지 말고 현상을 서술하라
(예: "M1" → "지속 관측된 흐름", "M4" → "여러 산업으로 번지는 양상").
등급은 확립/형성 중/관찰 중으로만 표현하라.

## 판정 사실(반영 필수 — 근거로만 사용, 코드 표기 금지)
- 등급: ${input.grade}
...
반드시 모두 포함하라. 그러면 분량이 자연히 ${LENGTH.IMPLICATION_MIN}자를 넘는다:
  ① 조건부 전개: "만약 <이 흐름이 함의하는 조건>이 지속·확대되면 <결과>" 형태.
     조건은 현상 서술로 쓰라(운동유형 코드·등급 코드를 문장에 넣지 말 것).
...
## 판단 철회 조건(killTrigger 필드) 규칙
- "만약 <관측 가능한 사건>이 <시점(날짜)>까지 발생하면 이 판단을 철회한다" 형식.
- 문장에 "킬 트리거"라는 표현을 쓰지 말 것(내부 용어).
