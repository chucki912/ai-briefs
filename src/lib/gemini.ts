...
1. **산업 구조 변화**: keyFacts 3개가 만들어내는 메커니즘이 글로벌 산업 구조에 어떤 동역학적 변화를 일으키는가
2. **분석 핵심**: 시장 구조 변화, 비즈니스 모델 압력, 기술 전이, 규제 모멘텀, 노동 구조 충격 중 가장 파괴적인 요소를 선정하여 심층 분석
...
- ❌ **순서 암시 단어 절대 금지**: "앞선 브리프에서", "이전에 논의된" 등 브리프의 읽는 순서를 단정 짓는 단어는 절대 쓰지 말고 수평적/동시다발적 인과로만 서술할 것.
...
[ ] 3. keyFacts가 '단 1개의 사건'만을 3문장으로 쪼개서 상세히 설명하고 있는가? (서로 다른 3개 사건 병렬 나열 금지)
[ ] 4. insight에 산업 구조 변화 / 분석 핵심 요인 / 처방 3요소가 있는가?
[ ] 5. insight가 이전 브리프의 맥락과 유기적으로 이어지는가? (기계적인 문구 삽입 없이 자연스럽게)
