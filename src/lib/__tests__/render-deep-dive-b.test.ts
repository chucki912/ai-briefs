...
// 6. aggregator tier 표기 (R4 이후: 미해석 소스는 URL 대신 도메인 힌트)
chk('애그리게이터 검증 필요 표기', md.includes('(원문 링크 미해석 — 도메인: tradethepool.com) (애그리게이터 — 검증 필요)'));

// 6b. R4 — 미해석/해석 소스 표기 정책
chk('미해석 소스: URL 대신 도메인 표기', md.includes('(원문 링크 미해석 — 도메인: trendforce.com)'));
chk('리다이렉트 원문 URL 미출력', !md.includes('vertexaisearch'));
chk('해석된 소스(s1)는 최종 URL 출력', md.includes('1. reuters.com — https://www.reuters.com/hbm4-cert'));
{
    const s = clone();
    s.sourceRefs[1] = { id: 'g1', url: 'https://www.trendforce.com/news/hbm4', outlet: 'trendforce.com', title: 'trendforce.com', resolved: true, tier: 'unknown' };
    const m = renderDeepDiveB(s);
    chk('해석 성공 grounding 소스는 최종 URL 출력', m.includes('trendforce.com — https://www.trendforce.com/news/hbm4'));
}
